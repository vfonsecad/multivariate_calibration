...
selected_model_id = 5; % but in position is model 4

chosen_nlv = current_lv_cv(selected_model_id);
chosen_alpha = final_rsimpls_alpha_range(selected_model_id);    
chosen_rsimpls_h = round(chosen_alpha*ncal,0) ;
mod_rpls = rsimpls(xcal_current,ycal_current,'k',chosen_nlv,'rmsecv',0,'plots',0,'rmsep',1, 'h',chosen_rsimpls_h );
mod_simpls = rsimpls(xcal_current,ycal_current,'k',chosen_nlv,'rmsecv',0,'plots',0,'rmsep',1, 'classic', 1 );
sample_weights = mod_rpls.flag.all;
%%

save(mainpath + "/data/d0001_corn/data_prepared/" + "rsimpls_model_0"+num2str(selected_model_id) + "_sample_flags.mat", 'sample_weights')


%%

% --- simpls scores to compare weights 
...
for ii=1:size(mod_rpls.flag.all,1)
    ...
    plot(mod_simpls.classic.T(ii,1),mod_simpls.classic.T(ii,2),'o', 'Color', cmp(color_id,:), 'LineWidth', 8)
    hold on
end
xlabel("lv 1")
ylabel("lv 2")
